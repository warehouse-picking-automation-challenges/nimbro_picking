        ...
        for item in pick_data['bin_contents'][bin_name]:
            if item in CONST_ITEM_NAMES:
                if item in items:
                    ...
                    found_items_bin.append(str(item))
                    items.remove(item)
                else:
...
if 'bin_contents' in stow_data:
    for bin_name in stow_data['bin_contents']:
        ...
        for item in stow_data['bin_contents'][bin_name]:
            if item in CONST_ITEM_NAMES:
                if item in items:
		    found_items_tote.append(str(item))
                    total_items = total_items + 1
                    items.remove(item)
                else:
                    print (bcolors.FAIL + "  ERROR: Item '%s' used too many times" +
...
if bin_counts[4] <> 0:
    print (bcolors.FAIL + '  ERROR: Found %d/%d 4-item bins'
           + bcolors.ENDC) % (bin_counts[4], 0)
    success = False
if bin_counts[5] <> 2:
    print (bcolors.FAIL + '  ERROR: Found %d/%d 5-item bins'
           + bcolors.ENDC) % (bin_counts[5], 2)
    success = False
if bin_counts[6] <> 0:
    print (bcolors.FAIL + '  ERROR: Found %d/%d 6-item bins'
           + bcolors.ENDC) % (bin_counts[6], 0)
...
if total_items <> len(CONST_ITEM_NAMES):
    print (bcolors.FAIL + "  ERROR: Accounted for %d/%d total items" +
           bcolors.ENDC) % (total_items, len(CONST_ITEM_NAMES))
    set_items = list(set(CONST_ITEM_NAMES))
    for item in set_items:
	counter = 0;
	for i in CONST_ITEM_NAMES:
	  if i == item:
	    counter = counter + 1
	counter_tote = 0
	for i in found_items_tote:
	  if i == item:
	    counter_tote = counter_tote + 1
	if (counter <> counter_tote):
	  print (bcolors.FAIL + "  ERROR: Missing %d instances of item '%s' " +
	    bcolors.ENDC) % (counter - counter_tote, item)
