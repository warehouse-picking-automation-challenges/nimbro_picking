...
const std::size_t RRLogger::MAX_MAX_NUM_BAGS = 100;       // 100 bags
const std::size_t RRLogger::MAX_MAX_TOTAL_MB = (1 << 20); // 1TB
...
// Constructor
RRLogger::RRLogger(const std::string& loggerType)
 : m_nhs()
 , PLOT_TOPIC(m_nhs.resolveName("plot"))
 , TF_TOPIC(m_nhs.resolveName("tf"))
 , RESOURCE_PATH(ros::names::clean(m_nhs.getNamespace()) + "/")
 , CONFIG_PARAM_PATH("/" + loggerType + "/")
 , m_confEnable(CONFIG_PARAM_PATH + "enable", true)
 , m_confPause(CONFIG_PARAM_PATH + "pause", false)
 , m_confStartNewBag(CONFIG_PARAM_PATH + "startNewBag", false)
 , m_confForceHeartbeat(CONFIG_PARAM_PATH + "forceHeartbeat", false)
 , m_confKeepAllBags(CONFIG_PARAM_PATH + "keepAllBags", false)
 , m_confKeepNoBags(CONFIG_PARAM_PATH + "keepNoBags", false)
 , m_inited(false)
 , m_enabled(false)
 , m_started(false)
 , m_bagOpen(false)
 ...
 , m_showEnabledState(false)
{
	...
	// Reset the logger configuration (an initialised logger type is required for the reset() function below)
	resetConfig(loggerType);
	...
	m_sub_heartbeat = nh.subscribe<rrlogger::LoggerHeartbeat>("heartbeat", 10, &RRLogger::handleHeartbeat, this);

	// Create ROS timers
	m_heartbeatTimer = nh.createTimer(ros::Duration(1.0), boost::bind(&RRLogger::handleHeartbeatTimer, this, _1), true, false); // Note: One-shot timer without autostart
}
...
// Reset function
void RRLogger::reset(const std::string& loggerType)
{
...
void RRLogger::resetConfig(const std::string& loggerType)
{
	// Reset logger configuration
	m_maxNumBags = 3;
	m_maxTotalMB = 4096;
	configureLoggerType(loggerType);
...
void RRLogger::addTopic(const std::string& name, std::string* resolvedName)
{
	...
	// Try to resolve the topic name (remapping enabled, resolution relative to node namespace)
	std::string resolvedTopic;
	try { resolvedTopic = m_nhs.resolveName(name, true); }
	catch(std::exception& e)
	{
		std::ostringstream ss;
		ss << "Could not resolve topic '" << name << "': " << e.what();
	...
	{
		std::ostringstream ss;
		ss << "Not adding topic '" << resolvedTopic << "' resolved from '" << name << "' because it already exists in the topic list!";
		throw std::runtime_error(ss.str());
	}
	...
	try
	{
		if(resolvedTopic == PLOT_TOPIC)
			sub = m_nhs.subscribe(resolvedTopic, TOPIC_QUEUE_SIZE, &RRLogger::writePlotCb, this);
		else if(resolvedTopic == TF_TOPIC)
			sub = m_nhs.subscribe(resolvedTopic, TOPIC_QUEUE_SIZE, &RRLogger::writeTfCb, this);
		else
		{
			...
			sub = m_nhs.subscribe<topic_tools::ShapeShifter>(resolvedTopic, TOPIC_QUEUE_SIZE, callback);
		}
	}
	catch(std::exception& e)
	{
		std::ostringstream ss;
		ss << "Failed to subscribe to topic '" << resolvedTopic << "' resolved from '" << name << "': " << e.what();
		throw std::runtime_error(ss.str());
	}
	if(!sub)
	{
		std::ostringstream ss;
		ss << "Failed to subscribe to topic '" << resolvedTopic << "' resolved from '" << name << "' for an unknown reason!";
		throw std::runtime_error(ss.str());
	}
	...
	if(subscriberTopic != resolvedTopic)
		ROS_WARN("[%s] Topic mismatch inside the ROS subscriber ('%s' != '%s'), this should never happen!", m_loggerType.c_str(), subscriberTopic.c_str(), resolvedTopic.c_str());
...
bool RRLogger::addPlotTopic(const std::string& name, std::string* resolvedName)
{
	...
	// Try to resolve the plot topic name (remapping disabled, resolution relative to node namespace)
	std::string plotTopic = name;
	if(plotTopic[0] != '/')
		plotTopic.insert(0, RESOURCE_PATH);
...
void RRLogger::handleConfStartNewBag()
{
	...
	{
		m_confStartNewBag.set(false);
		ROS_INFO("[%s] Manual trigger to start a new bag has been received%s!", m_loggerType.c_str(), (m_bagOpen ? "" : ", although no logging is currently in progress"));
		stopLogging();
		updateLoggingState();
	}
}
...
void RRLogger::updateEnabled()
{
	...
	{
		ROS_INFO("[%s] Logger has been enabled...", m_loggerType.c_str());
		m_enabled = true;
		m_showEnabledState = false;
	...
	if(!shouldBeEnabled && (m_enabled || m_showEnabledState))
	{
		...
		m_showEnabledState = false;
		resetHeartbeat(); // Note: This calls updateLoggingState() internally
		ROS_INFO("[%s] Logger has been disabled...", m_loggerType.c_str());
	}
}
...
void RRLogger::updatePaused(bool showInfo)
{
	...
	if(shouldBePaused && m_paused && showInfo && m_started)
		ROS_INFO("[%s] Logging is paused...", m_loggerType.c_str());

	// Pause logging if required
	if(shouldBePaused && !m_paused)
	{
		if(m_started)
			ROS_INFO("[%s] Logging has been paused...", m_loggerType.c_str());
		m_paused = true;
	}
	...
	{
		if(m_started)
			ROS_INFO("[%s] Logging has been resumed...", m_loggerType.c_str());
		m_paused = false;
	}
}
...
void RRLogger::updateLoggingState()
{
	...
	if(shouldLog && !m_started)
	{
		if(haveSourceStates)
		{
			...
			ROS_INFO("[%s] Active heartbeat sources: %s", m_loggerType.c_str(), ss.str().c_str());
		}
		startLogging();
	}
	if(!shouldLog && m_started)
	{
		...
		{
			if(m_heartbeatOptional)
				ROS_INFO("[%s] All heartbeat sources say to stop logging!", m_loggerType.c_str());
			else
				ROS_INFO("[%s] All heartbeat sources either say to stop logging or have timed out!", m_loggerType.c_str());
		}
		stopLogging();
	}
...
bool RRLogger::startLogging()
{
	...
	ROS_INFO("[%s] Logging has started...", m_loggerType.c_str());
	...
	{
		ROS_ERROR("[%s] Failed to create a new bag in directory '%s'!", m_loggerType.c_str(), LOG_PATH.c_str());
		return false;
	}
}
...
void RRLogger::stopLogging()
{
	...
	if(m_started)
		ROS_INFO("[%s] Logging has stopped...", m_loggerType.c_str());
...
bool RRLogger::openBag()
{
	...
	std::time_t newTime;
	std::time(&newTime);
	if(std::strftime(buf, 32, "_%Y%m%d_%H%M%S", std::localtime(&newTime)) != 16)
	{
		ROS_ERROR("[%s] Unexpected error with std::strftime, this should never happen!", m_loggerType.c_str());
		return false;
	}
	std::string fileSuffix = ".bag";
	...
	{
		fileSuffix = "_" + nodeNs.substr(startIndex, endIndex - startIndex + 1) + fileSuffix;
		std::replace(fileSuffix.begin(), fileSuffix.end(), '/', '_');
	}
	std::string newBagFileName = m_loggerType + std::string(buf) + fileSuffix;
	...
	if(!m_bagStamps.empty() && m_bagStamps.full() && (now - m_bagStamps.front()).toSec() < WARN_BAGS_SEC)
		ROS_WARN("[%s] Warning: Creating more than %d bags in %.1f seconds!", m_loggerType.c_str(), (int)m_bagStamps.capacity(), WARN_BAGS_SEC);
	...
	try
	{
		...
		{
			ROS_ERROR("[%s] The logging directory '%s' does not exist or is not a directory!", m_loggerType.c_str(), dir.string().c_str());
			return false;
		}

		// Look what other bags of ours are already in the logging directory
		if(m_confKeepAllBags())
			ROS_WARN("[%s] Not deleting any old bags because the config server says to keep all of them!", m_loggerType.c_str());
		else
		{
			// Set up the regex to filter the logging directory contents by
			const boost::regex fileNameRegex("^(.*)_([0-9]{8})_([0-9]{6})_?[a-zA-Z0-9_]*\\.bag$");
			...
			for(; file != end; ++file)
			{
				...
				if(!boost::regex_search(fileName, match, fileNameRegex)) continue;

				// Extract the components of the file name
				if(match.size() < 4)
				{
					ROS_ERROR("[%s] Logical problem with the bag filename regex (only %d sub-expression matches), this should never happen!", m_loggerType.c_str(), (int)match.size());
					continue;
				}
				std::string loggerType = match.str(1);
				std::string date = match.str(2);
				std::string time = match.str(3);

				// Ignore bags that don't have our logger type
				if(loggerType != m_loggerType) continue;
				...
				{
					ROS_WARN("[%s] Failed to retrieve the size of the existing bag '%s'!", m_loggerType.c_str(), fileName.c_str());
					bagBytes = 0;
				}
				totalBytes += bagBytes;
			...
			if(m_confKeepNoBags())
			{
				ROS_WARN("[%s] Deleting all old bags because the config server says to keep none!", m_loggerType.c_str());
			...
			if(bagsToDelete > 0)
			{
				...
				for(std::size_t i = 0; i < bagsToDelete; i++)
				{
					...
					ROS_INFO("[%s] Deleting bag %s", m_loggerType.c_str(), oldBag.c_str());
					try
					{
						if(!fs::remove(oldBag))
							ROS_WARN("[%s] The bag to delete was not found!", m_loggerType.c_str());
					}
					catch(std::exception& e)
					{
						ROS_WARN("[%s] Bag deletion failed: %s", m_loggerType.c_str(), e.what());
					}
				}
			}
		}
		...
		ROS_INFO("[%s] Creating bag %s", m_loggerType.c_str(), newBagFilePath.c_str());
	...
	{
		ROS_ERROR("[%s] Failed to open bag: %s", m_loggerType.c_str(), e.what());
...
void RRLogger::closeBag()
{
	...
	if(!fileName.empty())
		ROS_INFO("[%s] Closed bag %s", m_loggerType.c_str(), fileName.c_str());
...
void RRLogger::handleHeartbeat(const LoggerHeartbeatConstPtr& msg)
{
	...
	{
		ROS_WARN_THROTTLE(1.0, "[%s] Received a heartbeat with an empty sourceNode field => Ignoring...", m_loggerType.c_str());
		return;
	}
	...
	if(sourceIt == m_heartbeatMap.end())
		ROS_INFO("[%s] Heartbeat source '%s' says %sto log (NEW)", m_loggerType.c_str(), msg->sourceNode.c_str(), (msg->enableLogging ? "" : "NOT "));
	else if(msg->enableLogging != sourceIt->second.second)
		ROS_INFO("[%s] Heartbeat source '%s' says %sto log", m_loggerType.c_str(), msg->sourceNode.c_str(), (msg->enableLogging ? "" : "NOT "));
	...
	if(m_heartbeatMap.size() >= MAX_HEARTBEAT_SOURCES)
	{
		...
		if(discardIt != end)
		{
			ROS_WARN("[%s] Reached maximum number %d of maintained heartbeat sources => Discarding old heartbeat source '%s' to make room!", m_loggerType.c_str(), (int)MAX_HEARTBEAT_SOURCES, discardIt->first.c_str());
...
int main(int argc, char** argv)
{
	...
	// Extract the logger type from the node name (should only contain [0-9a-zA-Z_] in the end)
	std::string loggerType;
	std::string nodeName = ros::names::clean(ros::this_node::getName());
	std::size_t indexEnd = nodeName.find_last_not_of('/');
	if(indexEnd != std::string::npos)
	{
		std::size_t indexStart = nodeName.find_last_of('/', indexEnd);
		if(indexStart == std::string::npos)
			loggerType = nodeName.substr(0, indexEnd);
		else
			loggerType = nodeName.substr(indexStart + 1, indexEnd - indexStart);
	}

	// Construct the logger
	RRLogger* rrlogger = new RRLogger(loggerType);
	...
	std::size_t maxNumBags = (std::size_t) (maxNumBagsInt >= 0 ? maxNumBagsInt : 0);
	std::size_t maxTotalMB = (std::size_t) (maxTotalMBInt >= 0 ? maxTotalMBInt : 0);
	...
	if(heartbeatEnabled)
		ROS_INFO("[%s] RRLogger configured with %s heartbeat of timeout %.2fs (max %d bags / %dMB)", loggerType.c_str(), (heartbeatOptional ? "an optional" : "a mandatory"), heartbeatTimeout, (int)maxNumBags, (int)maxTotalMB);
	else
		ROS_INFO("[%s] RRLogger configured without a heartbeat (max %d bags / %dMB)", loggerType.c_str(), (int)maxNumBags, (int)maxTotalMB);
	...
	if(nh.getParam(topicsParam, list))
	{
		...
		{
			ROS_ERROR("[%s] The '%s' parameter is invalid (not of type XmlRpcValue::TypeArray) => Exiting...", loggerType.c_str(), topicsParam.c_str());
			return -1;
		}
		try
		{
			for(int i = 0; i < list.size(); i++)
			{
				...
				if(value.getType() == XmlRpc::XmlRpcValue::TypeString)
				{
					...
					if(topicName.empty())
						ROS_WARN("[%s] Ignored empty topic name!", loggerType.c_str());
					else
					{
						try
						{
							...
							ROS_INFO("[%s] Added topic: %s", loggerType.c_str(), resolvedTopicName.c_str());
							...
							if(resolvedTopicName == rrlogger->PLOT_TOPIC)
							{
								...
								if(nh.getParam(plotTopicsParam, plotList))
								{
										...
										for(int j = 0; j < plotList.size(); j++)
										{
											...
											if(plotValue.getType() == XmlRpc::XmlRpcValue::TypeString)
											{
												...
												if(plotTopicName.empty())
													ROS_WARN("[%s]  - <empty> (IGNORED)", loggerType.c_str());
												else
												{
													if(rrlogger->addPlotTopic(plotTopicName, &resolvedPlotTopicName))
														ROS_INFO("[%s]  - %s", loggerType.c_str(), resolvedPlotTopicName.c_str());
													else
														ROS_WARN("[%s]  - %s (FAILED)", loggerType.c_str(), resolvedPlotTopicName.c_str());
												}
											}
											else
											{
												ROS_WARN("[%s] The '%s' parameter contains non-string values!", loggerType.c_str(), plotTopicsParam.c_str());
									...
									{
										ROS_WARN("[%s] The '%s' parameter is invalid (not of type XmlRpcValue::TypeArray)!", loggerType.c_str(), plotTopicsParam.c_str());
										removePlotSubscriber = true;
									}
								}
								else
								{
									ROS_WARN("[%s] The plot topic '%s' has been specified, but the required '%s' parameter does not exist on the parameter server => Logging ALL plot topics by default...", loggerType.c_str(), resolvedTopicName.c_str(), plotTopicsParam.c_str());
									if(rrlogger->addPlotTopic("/"))
										ROS_WARN("[%s]  - /", loggerType.c_str());
									else
										ROS_WARN("[%s]  - / (FAILED)", loggerType.c_str());
										
								}
								if(removePlotSubscriber)
								{
									rrlogger->removeTopic(resolvedTopicName);
									ROS_WARN("[%s] Removed topic: %s", loggerType.c_str(), resolvedTopicName.c_str());
								}
							}
							...
							if(resolvedTopicName == rrlogger->TF_TOPIC)
							{
								...
								if(nh.getParam(tfFramesParam, tfList))
								{
										...
										for(int j = 0; j < tfList.size(); j++)
										{
											...
											if(tfFrameValue.getType() == XmlRpc::XmlRpcValue::TypeString)
											{
												...
												if(rrlogger->addTfFrame(tfFrame, &resolvedTfFrame))
													ROS_INFO("[%s]  - %s", loggerType.c_str(), (resolvedTfFrame.empty() ? "<all>" : resolvedTfFrame.c_str()));
												else
													ROS_WARN("[%s]  - %s (FAILED)", loggerType.c_str(), (resolvedTfFrame.empty() ? "<all>" : resolvedTfFrame.c_str()));
											}
											else
											{
												ROS_WARN("[%s] The '%s' parameter contains non-string values!", loggerType.c_str(), tfFramesParam.c_str());
									...
									{
										ROS_WARN("[%s] The '%s' parameter is invalid (not of type XmlRpcValue::TypeArray)!", loggerType.c_str(), tfFramesParam.c_str());
										removeTfSubscriber = true;
									}
								}
								else
								{
									ROS_WARN("[%s] The tf topic '%s' has been specified, but the required '%s' parameter does not exist on the parameter server => Logging ALL tf frames by default...", loggerType.c_str(), resolvedTopicName.c_str(), tfFramesParam.c_str());
									if(rrlogger->addTfFrame(""))
										ROS_WARN("[%s]  - <all>", loggerType.c_str());
									else
										ROS_WARN("[%s]  - <all> (FAILED)", loggerType.c_str());
								}
						...
						{
							ROS_WARN("[%s] %s", loggerType.c_str(), e.what());
							ROS_WARN("[%s] Failed to add topic '%s'!", loggerType.c_str(), topicName.c_str());
						}
					}
				}
				else
		...
		{
			ROS_ERROR("[%s] %s", loggerType.c_str(), e.what());
			ROS_ERROR("[%s] Failed to compile the list of topics to log => Exiting...", loggerType.c_str());
			return -1;
		}
	}
	else
		ROS_ERROR("[%s] The required '%s' parameter does not exist on the parameter server!", loggerType.c_str(), topicsParam.c_str());

	// Error out if there are no topics to log
	if(rrlogger->numTopics() <= 0)
	{
		ROS_ERROR("[%s] No topics have been specified to log => Exiting...", loggerType.c_str());
		return -1;
	}

	// Indicate that we have finished the initialisation
	ROS_INFO("[%s] RRLogger initialisation complete", loggerType.c_str());
	...
	{
		ROS_ERROR("[%s] Caught ROS bag exception: %s => Exiting...", loggerType.c_str(), e.what());
		return -1;
	}
	catch(std::exception& e)
	{
		ROS_ERROR("[%s] Caught generic std exception: %s => Exiting...", loggerType.c_str(), e.what());
		return -1;
	}
	...
	ROS_INFO("[%s] RRLogger is exiting cleanly...", loggerType.c_str());
