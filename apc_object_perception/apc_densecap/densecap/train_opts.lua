...
function M.parse(arg)
  ...
  cmd:option('-depth', 0,
    'Use depth in network')
  cmd:option('-depth_cnn', 'rgb', 'Depth CNN checkpoint to load. Use "rgb" to use RGB network. Use "" to concat HHA.')
  cmd:option('-cluster_anchors', 0, 'Cluster anchors?')
  ...
  cmd:option('-augment', 1, 'Enable data augmentation')
  ...
  cmd:option('-finetune_rpn_after', -1,
    'Start finetuning RPN after this many iterations (-1 = never finetune)')
  ...
  cmd:option('-evaluate_test', 1, 'Evaluate on test set?')
  cmd:option('-evaluate_val', 1, 'Evaluate on val set?')
  cmd:option('-evaluate_train', 0, 'Evaluate on training set?')
