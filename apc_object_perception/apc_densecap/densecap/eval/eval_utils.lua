...
function eval_utils.eval_split(kwargs)
  ...
  while true do
    ...
    local boxes, logprobs = model:forward_test(data.image)
    evaluator:addResult(logprobs, boxes, gt_boxes[1])
...
local function pluck_boxes(ix, boxes)
...
function DenseCaptioningEvaluator:addResult(logprobs, boxes, target_boxes)
  ...
  local merged_boxes = pluck_boxes(mergeix, target_boxes)
  ...
  for d=1,nd do -- for each detection in descending order of confidence
    ...
    record.ov = ovmax
    record.candidate = ii
    record.references = jmax -- will be nil if jmax stays -1
