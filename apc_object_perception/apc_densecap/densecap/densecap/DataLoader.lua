...
local augmentation = require 'densecap.augmentation'
...
function DataLoader:__init(opt)
  ...
  self.depth = utils.getopt(opt, 'depth', 0)
...
--   self.info.idx_to_token = idx_to_token
  ...
  table.insert(keys, 'orig_bbox')
  for k,v in pairs(keys) do
    print('reading ' .. v)
    self[v] = self.h5_file:read('/' .. v):all()
  end

  if pcall(function() self['cluster_anchors'] = self.h5_file:read('/cluster_anchors'):all() end) then
    print('Found cluster anchors!')
  else
    print('No cluster anchors.')
    self.cluster_anchors = nil
  end

  -- Labels are expected as 2-dim tensor
  self.labels:resize(self.labels:size(1), 1)
  ...
  if self.depth == 0 then
    assert(self.num_channels == 3, 'num_channels should be 3 if depth is not enabled')
  else
    assert(self.num_channels == 6, 'num_channels should be 6 if depth is enabled')
  end
  ...
  if self.depth == 0 then
    self.vgg_mean = torch.FloatTensor{103.939, 116.779, 123.68} -- BGR HHA order HHA: 92.2582016, 50.83613968, 98.79866028 - does not seem to help
    self.vgg_mean = self.vgg_mean:view(1,3,1,1)
  else
    -- Try to read HHA mean from json
    local hha_mean = torch.FloatTensor{103.939, 116.779, 123.68}
    if self.info.hha_mean then
      hha_mean = torch.FloatTensor(self.info.hha_mean)
      print('Found HHA mean in json:\n', hha_mean)
    else
      print('WARNING: Did not find HHA mean in json, using default BGR mean')
    end

    self.vgg_mean = torch.cat(
      torch.FloatTensor{103.939, 116.779, 123.68}, -- BGR HHA order
      hha_mean
    )
    self.vgg_mean = self.vgg_mean:view(1,6,1,1)
  end
...
function DataLoader:getLabelStrings()
  return self.info.label_strings
end
...
function DataLoader:getBatch(opt)
  ...
  local transform = utils.getopt(opt, 'transform', false)
  ...
  -- if requested, randomly transform (mirror) the image
  local transformed = 0
  if transform then
    -- clone annotations so that we don't modify the original ones
    box_batch = box_batch:clone()
    label_array = label_array:clone()

    img[1], box_batch, label_array = augmentation.augment(img[1], box_batch, label_array)
    transformed = 3
  end
  ...
  box_batch = box_batch:view(1, box_batch:size(1), box_batch:size(2)):clone()

  local VIS_TRANSFORM = false
  if VIS_TRANSFORM then
    local vis = img[1]:clone()
    -- swap RGB
    local perm = torch.LongTensor{3,2,1}
    vis = vis:index(1, perm)
    local boxes_draw = box_utils.xcycwh_to_x1y1x2y2(box_batch[1])
    for k=1,boxes_draw:size(1) do
    image.drawRect(vis, boxes_draw[k][1], boxes_draw[k][2], boxes_draw[k][3], boxes_draw[k][4], {color={0,255,0}, lineWidth=1, inplace=true})
    end
    self.win2 = image.display{image=vis, win=self.win2, legend='mirror', gui=false}
  end
  ...
  local info_table = { {filename = filename, 
                        ...
                        width = w, height = h, ori_width = ow, ori_height = oh, transformed = transformed,
                        ori_bbox = self.orig_bbox[ix]} }
