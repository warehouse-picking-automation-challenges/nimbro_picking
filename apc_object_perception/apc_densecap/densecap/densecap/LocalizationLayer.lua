...
function layer:__init(opt)
  ...
  self.overrideBoxes = false -- Set to override boxes
  self:reset_stats()
...
--  if self.overrideBoxes then
--    print('Overriding boxes!')
--    rpn_boxes_nms = self.overrideBoxesData
--  else
    local rpn_out
    ...
    if self.overrideBoxes then
      rpn_boxes_nms = torch.cat({rpn_boxes_nms, self.overrideBoxesData}, 1)
      scores_nms = torch.cat({scores_nms, torch.CudaTensor(self.overrideBoxesData:size(1)):fill(-0.5)}, 1)
    end
--  end
