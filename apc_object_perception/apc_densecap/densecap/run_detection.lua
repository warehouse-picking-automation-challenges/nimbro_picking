...
function lua_render_result(result, opt, outPath, data_image)
  local rgbIm
  ...
  if false then
    ...
    local rgbFile = paths.concat(imgpath, "rgb.png")
    rgbIm = rotate180(image.load(rgbFile))

    local crop_x1 = 1920 - 1 - (result.info.ori_bbox[1] + result.info.ori_bbox[3]/2)
    local crop_x2 = 1920 - 1 - (result.info.ori_bbox[1] - result.info.ori_bbox[3]/2)
    local crop_y1 = 1080 - 1 - (result.info.ori_bbox[2] + result.info.ori_bbox[4]/2)
    local crop_y2 = 1080 - 1 - (result.info.ori_bbox[2] - result.info.ori_bbox[4]/2)
    ...
    image.save(paths.concat(outPath, "annotation.png"), annotationIm)
  else
    rgbIm = data_image
  end
  local imgpath = paths.dirname(result.info.filename)
  meta = io.open(paths.concat(outPath, "meta.txt"), 'w')
  meta:write(imgpath)
  meta:close()
  print(rgbIm:size())
  image.save(paths.concat(outPath, "input.png"), rgbIm)

  for num_boxes=1,5 do
    -- slice results accordingly
    local slicedBoxes = result.boxes_xcycwh[{{1, num_boxes}}]:clone()
    local slicedClasses = result.classes[{{1, num_boxes}}]:clone()
    ...
    if false then
      ...
      -- flip 180° and add offset
      slicedBoxes[{{},1}] = result.info.ori_bbox[1] + result.info.ori_bbox[3]/2 - slicedBoxes[{{},1}]
      slicedBoxes[{{},2}] = result.info.ori_bbox[2] + result.info.ori_bbox[4]/2 - slicedBoxes[{{},2}]

      -- flip again
      slicedBoxes[{{},1}] = 1920 - 1 - slicedBoxes[{{},1}]
      slicedBoxes[{{},2}] = 1080 - 1 - slicedBoxes[{{},2}]
    end

    slicedBoxes_xy = box_utils.xcycwh_to_xywh(slicedBoxes)

    -- render
    local outImg = vis_utils.densecap_draw(rgbIm, slicedBoxes_xy, captions_sliced, {box_width=4}, slicedClasses)

    -- crop to box
--    outImg = image.crop(outImg, crop_x1, crop_y1, crop_x2, crop_y2)
...
--     end
	...
	if opt.output_dir ~= '' then
		...
		lua_render_result(result, opt, img_out_path, data.image:clone():float():add(1, loader.vgg_mean:expandAs(img)):mul(1.0/255.0):select(1,1):index(1, torch.LongTensor{3,2,1}))
	end
